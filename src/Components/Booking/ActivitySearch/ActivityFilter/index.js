...
const ActivityFilter = props => {
  ...
  const {onSelectData, onResetNow, state, setState} = route?.params;
  ...
  return (
    <Box flex={1} mb="m">
      <Header close title={'Filter options'} />
      <ScrollView
        ...
        <Button
          RenderComponent={() => (
            <Box
              ...
              <Text
                ...
                {loc ? loc?.info?.address : 'Select Location'}
              </Text>
              {loc && (
                <Box marginRight={'mt'}>
                  {Ionicon('close', 20, palette?.blackshade)}
          ...
          buttonStyle={TypographyStyles?.cardShadow}
        />
        <Text variant={'blackshade16600'} marginVertical="l">
          Radius
        </Text>
