...
const ActivityLeft = () => {
  ...
  const reqBody = {status: 3, per_page: 5, page: pagecount};
  const getActivityQuery = useQuery(['getActivity', reqBody], getActivity, {
    onSuccess: result => {
      ...
      setActivity({
        ...activity,
        transaction_data:
          activity.transaction_data.concat(result?.data?.data) || [],
              ...
              <Box flex={1}>
                {activity?.transaction_data.length === 0 && !loading ? (
                  <Box
                    ...
                    alignItems={'center'}>
                    <Text variant="blackshade116500">No Left Activities</Text>
                  </Box>
                ) : (
